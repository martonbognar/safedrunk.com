...
class Beverages extends Component {
  ...
  handleSubmit(event) {
    event.preventDefault();
    const self = this;
    const beverage = {'name': this.state.name, 'percentage': this.state.percentage, 'pending': this.state.submit};
    if (this.state.mixed) {
      beverage.mixed = true;
    ...
    axios.post(`/api/beverages`, beverage)
        .then(function(response) {
          ...
          const beverages = self.state.beverages.concat([beverage]);
  ...
  remove(id) {
    const confirmed = confirm('Are you sure you want to delete this beverage? This will also delete all logged drinks with this beverage.');
    if (!confirmed) {
      return;
    }
    ...
    const temp = this.state.beverages;
    temp.splice(index, 1);
  ...
  render() {
    ...
    const percentage = <div className="form-group">
    ...
    const ingredientList = this.state.ingredients.map((beverage) => <li key={beverage.id}>{beverage.name} {beverage.amount} {UNITS[beverage.unit]['name']}</li>);

    const mixedForm = <div><form>
      ...
      <hr />
            Add new ingredient:
      <BeverageLookup onBeverageSelect={(beverage) => {
        this.setState((beverage != undefined) ? {ingredientId: beverage.id, ingredientName: beverage.name, ingredientPercentage: beverage.percentage} : {ingredientId: undefined, ingredientName: '', ingredientPercentage: ''});
